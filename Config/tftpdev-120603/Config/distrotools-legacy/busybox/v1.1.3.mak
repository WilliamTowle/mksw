# busybox v1.1.3		[ since v0.60.2, c.2002-11-11 ]
...
NTI_BUSYBOX_INSTTEMP=${NTI_TC_ROOT}/insttemp

ifeq ($(shell echo ${BUSYBOX_VER} | sed 's/^1\.1\.[0-3]$$/y/'),y)
NTI_BUSYBOX_EXTRACTED=${EXTTEMP}/${NTI_BUSYBOX_TEMP}/Makefile
else
NTI_BUSYBOX_EXTRACTED=${EXTTEMP}/${NTI_BUSYBOX_TEMP}/configure
endif
	...
	( cd ${EXTTEMP}/${NTI_BUSYBOX_TEMP} || exit 1 ;\
		(	case ${BUSYBOX_VER} in \
			...
			*) \
			...
			[ "${BUSYBOX_VER}" = '1.1.3' ] && echo '# CONFIG_FEATURE_SORT_BIG is not set' ;\
			echo 'CONFIG_STTY=y' ;\
			echo 'CONFIG_TTY=y' ;\
			echo 'CONFIG_SYNC=y' ;\
		...
		1.1.[03]|1.2.2.1) \
			...
			cat Rules.mak.OLD \
				| sed	' /HOSTCC/	s%g*cc%'${NUI_CCPREFIX}'cc% ; /[(]CROSS[)]/	s%$$(CROSS)%$$(shell if [ -n "$${CROSS}" ] ; then echo $${CROSS} ; else echo "'`echo ${NUI_CC_PREFIX}cc | sed 's/gcc$$//'`'" ; fi)% ' > Rules.mak || exit 1 ;\
