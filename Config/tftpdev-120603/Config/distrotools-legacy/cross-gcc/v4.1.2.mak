# cross-gcc v4.1.2		[ since v2.7.2.3, c.2002-10-14 ]
# last mod WmT, 2010-06-24	[ (c) and GPLv2 1999-2010 ]
...
include ${CFG_ROOT}/distrotools-legacy/cross-binutils/v2.17.mak
	  ...
	  CFLAGS='-O2' \
	  	...
	  	AR=$(shell echo ${NTI_GCC} | sed 's/g*cc$$/ar/') \
			...
			--with-sysroot=${CTI_TC_ROOT}'/usr/'${CTI_SPEC}'/' \
		        --with-local-prefix=${CTI_TC_ROOT}'/usr/'${CTI_SPEC}'/local/' \
	        	--disable-nls \
	        	--disable-libmudflap \
	        	--disable-libssp \
			  || exit 1 \
