# uClibc v0.9.3x		[ since v0.9.9, 2002-10-26 ]
...
## UCLIBC_HAS_WCHAR gives us multibyte support for X libs/apps
## UCLIBC_SUSV3_LEGACY gives us usleep() for busybox
...
${CTI_UCLIBC_DEV_CONFIGURED}:
	...
	echo "conf..." ;\
		...
		0.9.31*|0.9.32*) \
			...
			echo 'RUNTIME_PREFIX="/"' ;\
			echo 'CROSS_COMPILER_PREFIX="'${CTI_TC_ROOT}'/usr/bin/'${CTI_MIN_SPEC}'-"' ;\
			echo '# HAS NO THREADS is not set' ;\
			echo 'LINUXTHREADS_OLD=y' ;\
...
${CTI_UCLIBC_DEV_INSTALLED}:
	...
	( cd ${EXTTEMP}/${CTI_UCLIBC_DEV_TEMP} || exit 1 ;\
		...
		0.9.31*|0.9.32*) \
			...
			( cd ${CTI_TC_ROOT}/usr/${CTI_SPEC}/usr/lib || exit 1 ;\
				echo "!! AVOID ... for SOBJ in *.so ; do [ -L $${SOBJ} ] && ln -sf $${SOBJ}.0 $${SOBJ} ; done" \
			) || exit 1 ;\
		;; \
		*) \
