...
build-cross:
	...
	cat Makefile.OLD \
		...
		| sed 's%^BIGFILES=.*%BIGFILES=-D_FILE_OFFSET_BITS=32%' \
		> Makefile
	PATH=${TOOLROOT}/usr/${TARGET_MACH}-linux-uclibc/bin:${PATH} make CROSS=${CROSS_PREFIX}- -f Makefile-libbz2_so
	#rm ${INSTTMP}/${PKGNAME}-${PKGVER}/usr/bin/bz*
	PATH=${TOOLROOT}/usr/${TARGET_MACH}-linux-uclibc/bin:${PATH} make CROSS=${CROSS_PREFIX}- PREFIX=${INSTTMP}/${PKGNAME}-${PKGVER}/usr install
