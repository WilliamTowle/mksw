# lhasa v0.3.1			[ since v0.2.0, 2015-04-09 ]
# last mod WmT, 2016-04-05	[ (c) and GPLv2 1999-2016 ]
...
LHASA_SRC=${SOURCES}/l/lhasa_0.3.1.orig.tar.xz
#URLS+= http://ftp.de.debian.org/debian/pool/main/l/lhasa/lhasa_0.2.0+git3fe46.orig.tar.xz
URLS+= http://www.mirrorservice.org/sites/ftp.debian.org/debian/pool/main/l/lhasa/lhasa_0.3.1.orig.tar.xz
...
${NTI_LHASA_EXTRACTED}:
	...
	[ ! -d ${EXTTEMP}/lhasa-${LHASA_VERSION} ] || rm -rf ${EXTTEMP}/lhasa-${LHASA_VERSION}
	...
	mv ${EXTTEMP}/lhasa-${LHASA_VERSION} ${EXTTEMP}/${NTI_LHASA_TEMP}
...
${NTI_LHASA_CONFIGURED}: ${NTI_LHASA_EXTRACTED}
	...
	( cd ${EXTTEMP}/${NTI_LHASA_TEMP} || exit 1 ;\
		...
		;; \
		*) \
			echo "Unexpected LHASA_VERSION ${LHASA_VERSION}" 1>&2 ;\
			exit 1 \
		;; \
		esac \
	)
