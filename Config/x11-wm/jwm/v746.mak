# jwm v746			[ since v2.0.1, c.2011-08-12 ]
...
# [2013-05-01, v746] force configure to use toolchain's 'pkg-config'

${NTI_JWM_CONFIGURED}: ${NTI_JWM_EXTRACTED}
	echo "*** $@ (CONFIGURED) ***"
	( cd ${EXTTEMP}/${NTI_JWM_TEMP} || exit 1 ;\
		[ -r configure.OLD ] || mv configure configure.OLD || exit 1 ;\
		sed '/which pkg-config/,/PKGCONFIG=/ s%pkg-config%'${NTI_TC_ROOT}'/usr/bin/'${HOSTSPEC}'-pkg-config%' \
