# openssl v1.1.0e		[ since v0.9.7b, c.2003-05-28 ]
...
${NTI_OPENSSL_CONFIGURED}: ${NTI_OPENSSL_EXTRACTED}
	...
	( cd ${EXTTEMP}/${NTI_OPENSSL_TEMP} || exit 1 ;\
			...
			./Configure linux-generic32 386 shared no-idea no-mdc2 no-rc5 zlib-dynamic \
			 ...
			 ;; \
			x86_64) \
			./Configure linux-${HOSTCPU} shared no-idea no-mdc2 no-rc5 zlib-dynamic || exit 1 \
			;; \
			*) \
		...
		1.1.0e) \
			cat Makefile.NEW \
				| sed '/^ENGINESDIR/	s%/usr%'${NTI_TC_ROOT}'/usr%' \
...
## [2016-02-01] Preliminary 'make depend' mandatory for all v1.1.0x
