# hostapd v2.5			[ since v2.1, c.2014-02-28 ]
# last mod WmT, 2016-08-02	[ (c) and GPLv2 1999-2016* ]
...
HOSTAPD_PATCHES= ${SOURCES}/h/hostapd_2_5.zip
URLS+= https://github.com/pritambaral/hostapd-rtl871xdrv/archive/hostapd_2_5.zip

ifneq (${HOSTAPD_PATCHES},)
# patches are zip-compressed:
include ${CFG_ROOT}/tools/unzip/v60.mak
endif

#include ${CFG_ROOT}/network/openssl/v1.0.2g.mak
include ${CFG_ROOT}/network/openssl/v1.0.2h.mak
...
ifneq (${HOSTAPD_PATCHES},)
	mkdir -p ${EXTTEMP}/hostapd-${HOSTAPD_VERSION}/patches
	( cd ${EXTTEMP}/hostapd-${HOSTAPD_VERSION}/patches ;\
		unzip -d . ${HOSTAPD_PATCHES} \
	)
endif
...
${NTI_HOSTAPD_CONFIGURED}: ${NTI_HOSTAPD_EXTRACTED}
	...
	( cd ${EXTTEMP}/${NTI_HOSTAPD_TEMP} || exit 1 ;\
		...
		cat hostapd/defconfig \
			...
			| sed '/DRIVER_WIRED/		s/^#*/#/ ' \
			| sed '/CONFIG_LIBNL20/		s/^#*/#/ ' \
			| sed '/CONFIG_LIBNL32/		s/^#*// ' \
			> hostapd/.config \
	)
ifneq (${HOSTAPD_PATCHES},)
	( cd ${EXTTEMP}/${NTI_HOSTAPD_TEMP} || exit 1 ;\
		patch -Np1 -i patches/hostapd-rtl871xdrv-hostapd_2_5/rtlxdrv.patch || exit 1 ;\
		echo '# CUSTOM - patched by mksw build:\nCONFIG_DRIVER_RTW=y' >> hostapd/.config \
	)
endif
