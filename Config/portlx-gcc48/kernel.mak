...
URLS+= https://www.kernel.org/pub/linux/kernel/v4.x/linux-${KERNEL_VERSION}.tar.gz

# dependencies: there are only 'nti' dependencies for headers
# TODO: the full kernel build requires the 'kgcc' compiler

ifeq (${TARGCPU},mipsel)
CTI_KERNEL_ARCH=mips
else
CTI_KERNEL_ARCH=${TARGCPU}
endif
...
ifeq (${CTI_KERNEL_ARCH},mips)
CUI_KERNEL_BUILT	= ${EXTTEMP}/${CUI_KERNEL_SUBDIR}/vmlinux
else
CUI_KERNEL_BUILT	= ${EXTTEMP}/${CUI_KERNEL_SUBDIR}/arch/${CTI_KERNEL_ARCH}/boot/zImage
endif
...
# [2017-01-19] Enforce CONFIG_AEABI for arm (compiler with -uclibceabi)
# [2017-01-24] Enforce BLK_DEV_RAM (for qemu with squashfs initrd)
	...
	( cd ${EXTTEMP}/${CTI_LXHEADERS_SUBDIR} || exit 1 ;\
		...
		mips) \
			cat arch/mips/configs/ip22_defconfig \
				| sed	'/CPU_R5000/	s/R5000/R4X00/' \
				| sed	'/CONFIG_SERIAL_IP22_ZILOG/	s/=m/=y/' \
				| sed	'/CONFIG_SERIAL_CORE/		s/=m/=y/' \
				> .config || exit 1 ;\
			grep 'CPU' .config ;\
			echo	'CONFIG_SERIAL_8250=y' >> .config ;\
			echo	'CONFIG_SERIAL_8250_CONSOLE=y' >> .config ;\
			echo	'# CONFIG_UNIX98_PTYS=n' >> .config ;\
			echo	'#... CONFIG_SERIAL_IP22_ZILOG_CONSOLE=y' >> .config ;\
			echo	'CONFIG_SERIAL_CORE_CONSOLE=y' >> .config \
		;; \
		mipsel) \
			cp arch/mips/configs/malta_defconfig .config || exit 1 \
		;; \
		*) \
		...
		scripts/config --enable BLK_DEV_INITRD --enable BLK_DEV_RAM ;\
